    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(len(nums)):
            ...
            while l < r:
                ...
                elif -nums[i] < nums[l] + nums[r]: # -(-1) < -1 + 4 Our right number is too big so we reduce 
                    r -= 1
                elif -nums[i] > nums[l] + nums[r]: # Do the opposite of above 
                    l += 1
                # We moved our pointers so let our loop go 
        return res
